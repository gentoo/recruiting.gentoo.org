Given(/^sample question categories exist$/) do
  ["Developer Quiz", "Staffer Quiz"].inject(1) do |question_count, group_name|
    ...
    FactoryGirl.create :question, group: group, content: "Organizational structure questions #{question_count}"
    FactoryGirl.create :question, group: group, content: "Organizational structure questions #{question_count + 1}"
    question_count + 2
  end
end
